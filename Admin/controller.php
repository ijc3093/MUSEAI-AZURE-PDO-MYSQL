    ...
    class Controller{
        ...
        function get_Manager_museais($id){
            try{
                ...
                if( $stmt != false ){
                    while(($row = $stmt->fetch())){
                        $query2 = "select idmuseai, name, artist, whoisposted, year, datepost, time from museai WHERE idmuseai = " . $row['museai'] . ";";
        ...
        function insert_museai(
            ...
            try{
                $stmt = $this->dbh->prepare("insert into museai(
                    name, 
                    artist, 
                    year, 
                    datepost,
                ...
                if($mId = intval($this->dbh->lastInsertId())){
                    ...
                    $stmt1 = $this->dbh->prepare("insert into manager_museai (museai, manager) VALUES(?, ?);");
                    ...
                    //echo "event ID: " . $mId . "\n";
                    //echo "manager ID: " . $role_ID . "\n";
                    $stmt2 = $this->dbh->prepare("insert into ntid_museai (museai, ntid) VALUES(?, ?);");
        ...
        //user store data in database server (insert into qrCode_Image)
        function insert_qrCode($name, $qrCode_Image){
            $defaultErrorName = "error";
            try{
                $stmt = $this->dbh->prepare("insert into qrCode(name, qrCode_Image) VALUES(?, ?);");

                $stmt->bindParam(1, $name, PDO::PARAM_STR);
                $stmt->bindParam(2, $qrCode_Image, PDO::PARAM_STR);
        ...
        //Attend museai
        function attending_museai($museai, $user){
            try{
                //$stmt = $this->$dbh->prepare("insert into user set name = ?, password = ? role = ?;");
                $query = "insert into user_session (museai, user) values(?,?)";
                $stmt = $this->dbh->prepare($query);
                $stmt->bindParam("si", $museai, $user);
                if(!$stmt->execute()){
                    $return = -1;
                }
                else{
